...
const RezervasyonDuzenle: React.FC = () => {
    ...
    // Rezervasyon verisini çek ve formu doldur
    useEffect(() => {
        if (!id) return;
        const fetchData = async () => {
            try {
                ...
                // 1. Fakülte listesini çek
                const fakulteRes = await fetch('http://10.15.0.15:8080/fakulteler', { headers });
                const fakulteList = fakulteRes.ok ? await fakulteRes.json() : [];
                setFakulteler(fakulteList);
                // 2. Rezervasyon verisini çek
                const res = await fetch(`http://10.15.0.15:8080/rezervasyonlar/${id}`, { headers });
                if (!res.ok) throw new Error('Rezervasyon alınamadı');
                const data = await res.json();
                const fakulteId = data.fakulte?.id ? String(data.fakulte.id) : '';
                setFakulte(fakulteId);
                // 3. Salon listesini çek
                let salonList: any[] = [];
                if (fakulteId) {
                    const salonRes = await fetch(`http://10.15.0.15:8080/salonlar?fakulteId=${fakulteId}`, { headers });
                    salonList = salonRes.ok ? await salonRes.json() : [];
                    setSalonlar(salonList);
                }
                // 4. Salon id'sini set et
                setSinif(data.salon?.id ? String(data.salon.id) : '');
                // 5. Diğer alanları set et
                setKullanimTuru(data.tur || '');
                setBaslik(data.baslik || '');
                setAciklama(data.aciklama || '');
                if (data.date) {
                    const tarihObj = new Date(data.date);
                    setTarih(tarihObj);
                    setSeciliTarih(tarihObj.getDate());
                    // --- EKLENDİ: İlk açılışta mevcut rezervasyonları getir ---
                    if (fakulteId) {
                        rezervasyonlariGetir(tarihObj.getDate(), fakulteId);
                    }
                }
                setBaslangicSaati(data.startTime?.slice(0, 5) || '13:00');
                setBitisSaati(data.endTime?.slice(0, 5) || '14:00');
            } catch (e) {
                setMesaj('Hata: Rezervasyon verisi alınamadı.');
            }
        };
        fetchData();
    }, [id]);
    ...
    const rezervasyonGuncelle = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // PUT ile güncelle
            const res = await fetch(`http://10.15.0.15:8080/rezervasyonlar/${id}`, {
                method: 'PUT',
                headers: {
        ...
        <div style={{ width: '100%', height: '100%', background: 'white' }}>
                        ...
                        <div className="takvim-grup">
                            ...
                            <div className="saat-ve-rezervasyon-gruplari">
                                ...
                                <div className="rezervasyon-grubu">
                                    <h3>Seçilen Gün ve Fakültede Ait Mevcut Rezervasyonlar</h3>
            ...
            {mesaj && (
                <div className="rezervasyon-toast" style={{
                    ...
                    {mesaj}
                    {mesaj.includes('başarıyla güncellendi') && (
